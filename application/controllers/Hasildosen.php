...
class Hasildosen extends CI_Controller {

   protected $page_header = 'Hasil Dosen Management';
   ...
   public function __construct()
   {
      ...
      $this->load->model(array('Hasildosen_model'=>'hasil', 'ThnAkademik_model'=>'akademik', 'Dosen_model'=>'dosen'));
	...
	{  
      ...
      $data['panel_heading'] = 'Hasil Dosen List';
      $data['page']         = '';

      $this->template->backend('hasildosen_v', $data);
	}

   public function get_hasildosen()
   {
      if (!$this->ion_auth->logged_in()){            
         redirect('auth/login', 'refresh');
      }
      ...
      $output = array(
         "draw" => $draw,
         "recordsTotal" => $this->hasil->count_rows(),
   ...
   public function form_data()
   {
      ...
      $data = array('hidden'=> form_hidden('aksi', !empty($row->dsnKelasID) ? 'update' : 'create'),
             'dsnKelasID' => form_input(array('name'=>'dsnKelasID', 'id'=>'datepicker', 'class'=>'form-control', 'value'=>!empty($row->dsnKelasID) ? $row->dsnKelasID : $uni)),
   ...
   {   
      ...
      $rules = array(
            'insert' => array(                     
                     array('field' => 'dsnKelasID', 'label' => 'Dosen Kelas ID', 'rules' => 'trim|required|is_unique[dosen_kelas.dsnKelasID]|max_length[12]'), 
            ...
            'update' => array(
                     array('field' => 'dsnKelasID', 'label' => 'Dosen Kelas ID', 'rules' => 'trim|required|max_length[12]'), 
                     array('field' => 'kelasID', 'label' => 'Nama Kelas', 'rules' => 'max_length[11]'),
                     array('field' => 'npp', 'label' => 'Nama Dosen', 'rules' => 'max_length[11]'),
                     array('field' => 'thnAkademikID', 'label' => 'Tahun Akademik', 'rules' => 'max_length[20]')
                     ),             
